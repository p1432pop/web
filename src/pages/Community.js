...
export default function Community(props) {
		...
		for(let i=1; i<=69; i++) {
			arr.push(
				<FormControlLabel key={i}
					...
					label={<Avatar alt="img" src={`/image/CharacterIcon/${i}.png`}></Avatar>}
				/>
			)
		}
		return arr;
	...
	return (
		<>
							...
							>
								...
								<StyledTableCell>
									<div className={styles.avatarBox}>
										{avatarImage([row.char1, row.char2, row.char3])}
									</div>
								</StyledTableCell>
			...
			<Button onClick={openModal} fullWidth variant="contained" sx={{ mt: 3, mb: 2 }} size="large">글쓰기</Button>
			{open ? 
			<Modal
				open={open}
				onClose={closeModal}
