...
export default function SignUp(props) {
	...
	const handleSubmit = (e) => {
		...
		if (!passwordRegex.test(password)) setPasswordState("비밀번호: 8~16자의 영문 대/소문자, 숫자, 특수문자를 사용해 주세요.");
		...
		// 인증 코드 검사
		if (validation !== validationCode) setValidationError("인증 코드가 일치하지 않습니다.");
		else setValidationError("");

		if (idRegex.test(id) && passwordRegex.test(password) && password === rePassword && validation === validationCode) {
