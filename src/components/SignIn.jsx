...
import { setLogin, setLogout, setWithdraw } from '../registerSlice';
    ...
    if (value.login) {
        return (
            <div>
                <Avatar src='' />
                <div>ID : {value.id}</div>
                <Button onClick={() => dispatch(setLogout())} type="submit" fullWidth variant="contained" sx={{ mt: 1, mb: 1 }} size="large">로그아웃</Button>
                <Button onClick={() => dispatch(setWithdraw(value.id))} type="submit" fullWidth variant="contained" sx={{ mt: 1, mb: 1 }} size="large">회원탈퇴</Button>
            </div>
        )
    }
    else {
